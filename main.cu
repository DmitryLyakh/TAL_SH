...
//Create and initialize 24 tensor blocks:
 //Tensor blocks [0-5] on first gpu:
 for(j=1;j<=8;j++){ //eight tensor contractions
  my_gpu=(j-1)/2; //expect 4 GPUs
  for(i=(j-1)*3;i<j*3;i++){
...
//Create empty CUDA task handles:
 for(i=0;i<MAX_TASKS;i++){
  errc=cuda_task_create(&(tsks[i])); if(errc){printf("#ERROR: cuda_task_create %d failed! \n",i); return 1;};
 };
...
//Clean CUDA tasks for reuse:
 for(i=0;i<k;i++){errc=cuda_task_clean(tsk[i]); if(errc){printf("#ERROR: cuda_task_clean %d failed!",i); return 1;};};

//Mark tensor blocks as absent on GPU:
 for(i=0;i<24;i++){errc=tensBlck_set_absence(tb[i]);};

//Choose the tensor transpose algorithm:
 gpu_set_transpose_algorithm(EFF_TRN_OFF); //turn off the efficient tensor transpose
 gpu_set_matmult_algorithm(BLAS_OFF); //switch to custom BLAS kernels
